...
def get_pie_chart(entered_site):
    if entered_site == 'ALL':
        ...
        successcount = sum(filtered_df['class'] == 1)
        failurecount = sum(filtered_df['class'] == 0)
        pie_df = pd.DataFrame({'class': ['Success', 'Failure'],
                               'count': [successcount, failurecount]})
        fig = px.pie(pie_df, values='count', names='class', title="Launch Success")
        return fig
# TASK 4:
